...
%先构建初始父代种群
for i = 1 : nPop
    Parent(i).x = 6*rand(1, 2);  %每一个个体的自变量是一个 1*2 的行向量 每一个xi 在0到6之间
...
%     
 ...
 disp(['迭代次数：', num2str(It), ', 最优解:', num2str(Parent(1).z)]); 
...
out = (input1*input1+input2-11)^2 + (input1+input2^2-7)^2 + ...
M2*((min(h1,0))^2+(min(h2,0))^2);
% +(min(h3,0))^2+(min(h4,0))^2+...
